                        ...
                        <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                @foreach ($data as $item)
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        <th scope="row"
                                            class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-900">
                                            {{ $item['NAME'] }}
                                        </th>
                                        <td class="px-6 py-4">
                                            @if ($item['Status'] == 0)
                                                <span id="{{ $item['ID'] }}"
                                                    class="flex h-7 bg-red-500 rounded-full mx-2 shadow-lg shadow-red-500/50"></span>
                                            @endif
                                            @if ($item['Status'] == 1)
                                                <span id="{{ $item['ID'] }}"
                                                    class="flex h-7 bg-yellow-300 rounded-full mx-2 shadow-lg shadow-yellow-300/50"></span>
                                            @endif
                                            @if ($item['Status'] == 2)
                                                <span id="{{ $item['ID'] }}"
                                                    class="flex h-7 bg-green-500 rounded-full mx-2 shadow-lg shadow-green-500/50"></span>
                                            @endif
                                        ...
                                        <td id="updated_{{ $item['ID'] }}"
                                            class="px-6 py-4 bg-gray-50 dark:bg-gray-900">
                                            {{ $item['UPDATED_AT'] == 0 ? 'Never' : $item['UPDATED_AT'] }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <button id="up_button_{{ $item['ID'] }}" type="button"
                                                onclick="refreshDepartment({{ $item['ID'] }}, '{{ $item['NAME'] }}')"
                            ...
                            <script>
                                function refreshDepartment(id, name) {
                                    ...
                                    var button = document.getElementById('up_button_' + id);
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            ...
                                            if (response !== "no") {
                                                ...
                                                if (info) {
                                                    info.className =
                                                        'flex h-7 bg-green-500 rounded-full mx-2 shadow-lg shadow-green-500/50';
                                                }
                                            } else {
