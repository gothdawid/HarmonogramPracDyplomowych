<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Single Calendar') . ' - ' . $calendar_name }}
        </h2>
    </x-slot>
    ...
    <div class="float-right w-1/6 min-w-120 max-w-200 mr-2 pr-5 dark:bg-gray-800 mt-12 rounded-lg">
        <div class="h-full px-3 py-4 overflow-y-auto flex flex-col">
            <p class="text-center text-white">{{ __('Calendar Options') }}</p>

            <div class="flex items-center mb-1 mt-4">
                <input id="show_lessons" type="checkbox" value=""
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label for="show_lessons"
                    class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Show defense commission lessons') }}</label>
            </div>

            <div class="flex items-center">
                <input id="prevent_overlapping_defenses" type="checkbox" value=""
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    checked>
                <label for="prevent_overlapping_defenses"
                    class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Prevent overlapping when same person on defense') }}</label>
            </div>

            <label for="timerange" class="block mb-2 mt-6 text-sm font-medium text-gray-900 dark:text-white">{{ __('Select time range for displayed lessons') }}</label>
            <select id="timerange" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="1" selected>{{ __('1 day') }}</option>
                <option value="2">{{ __('2 day') }}</option>
                <option value="3">{{ __('3 day') }}</option>
                <option value="4">{{ __('4 day') }}</option>
                <option value="5">{{ __('5 day') }}</option>
                <option value="6">{{ __('6 day') }}</option>
            ...
            <a class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 mt-7" data-modal-toggle="deleteModal">{{ __('Delete') }}</a>
